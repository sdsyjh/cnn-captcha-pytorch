...
if __name__ == '__main__':
    ...
    # test_model_path = "./model/check.epoch90"
    model.load_state_dict(torch.load(test_model_path, weights_only=True))
    model.eval()

    correct = 0  # 设置right变量，保存预测正确的样本数量
    all = 0  # all保存全部的样本数量
    # 遍历test_loader中的数据
    # x表示样本的特征张量，y表示样本的标签
    for data, label in test_loader:
        data, label = data.to(device), label.to(device)  # 转移数据至GPU
        output = model(data)  # 使用模型预测x的结果，保存在pred中
        for i in range(len(data)):
            ...
            if predicted == correctLabel:
                print(f"predicted = {predicted}, correctLabel = {correctLabel}")
                correct+=1
            else:
                print(f"predicted = {predicted}, correctLabel = {correctLabel}, wrong!!!")
        all += 1  # 每次循环，all变量加1

    # 循环结束后，计算模型的正确率
    acc = correct * 1.0 / all
    print("test accuracy = %d / %d = %.3lf" % (correct, all, acc))
